...
/**
 * Alternate msgpack lib to https://github.com/msgpack/msgpack-java The API is
 * very minimal and simplistic.
 * 
 * This implementation uses the builtin Java types: - null - Boolean - Number
 ...
 * - String (UTF-8), byte[], or ByteBuffer (the *whole* buffer) (always unpacked
 * as a byte[] unless you ask for something else) - Map (any type may be used
 * for packing, always unpacked as a HashMap) - List (any type may be used for
 * packing, always unpacked as an ArrayList) Passing any other types will throw
 * an IllegalArumentException.
 * 
 * @author jon
 */
public class MsgPack {
     ...
     * 
     * Warning: this does not do any recursion checks. If you pass a cyclic
     * object, you will run in an infinite loop until you run out of memory.
     * 
     * @param item
     * @return the packed data
     * @throws UnpackableItemException
     *             If the given data cannot be packed.
     */
    public static byte[] pack(Object item) throws UnpackableItemException {
        ...
        try {
            ...
            throw new RuntimeException(
                    "ByteArrayOutputStream threw an IOException!", ex);
        }
     ...
     * @param packed
     *            data
     * @param int options Bitmask of flags to specify how to map certain values
     *        back to java types: For raw types: (no option) - All raw bytes are
     *        decoded as a byte[] OPTION_RAW_AS_STRING - All raw bytes are
     *        decoded as a UTF-8 string, with invalid codepoints replaced with a
     *        placeholder OPTION_RAW_AS_BYTE_BUFFER - All raw bytes are decoded
     *        as ByteBuffers (with a backing array)
     * @return the unpacked data
     * @throws InvalidMsgPackDataException
     *             If the given data cannot be unpacked.
     */
    public static Object unpack(byte[] data, int options)
            throws InvalidMsgPackDataException {
        ...
        try {
            ...
            // this shouldn't happen
            throw new RuntimeException(
                    "ByteArrayInStream threw an IOException!", ex);
        }
    }
    ...
    protected static final byte MP_NEGATIVE_FIXNUM = (byte) 0xe0;// last 5 bits
                                                                 // is value
    protected static final int MP_NEGATIVE_FIXNUM_INT = 0xe0;// /me wishes for
                                                             // signed numbers.
    ...
    /**
     * Packs the item, streaming the data to the given OutputStream. Warning:
     * this does not do any recursion checks. If you pass a cyclic object, you
     * will run in an infinite loop until you run out of memory/space to write.
     * 
     * @param item
     * @param out
     */
    public static void pack(Object item, DataOutputStream out)
            throws IOException {
        if (item == null) {
            out.write(MP_NULL);
        ...
        } else if (item instanceof String || item instanceof byte[]
     ...
     * @param options
     *            Bitmask of options, @see unpack(byte[] data, int options)
     * @throws IOException
     *             if the underlying stream has an error
     * @throws InvalidMsgPackDataException
     *             If the given data cannot be unpacked.
     */
    public static Object unpack(DataInputStream in, int options)
            throws IOException {
        int value = in.read();
        if (value < 0)
            throw new InvalidMsgPackDataException(
                    "No more input available when expecting a value");
        ...
        try {
            ...
            switch ((byte) value) {
                ...
                case MP_UINT16:
                    return in.readShort() & MAX_16BIT;// read short, trick Java
                                                      // into treating it as
                                                      // unsigned, return int
                case MP_UINT32:
                    return in.readInt() & MAX_32BIT;// read int, trick Java into
                ...
                case MP_UINT64: {
                    ...
                    if (v >= 0)
                        ...
                        // this is a little bit more tricky, since we don't have
                        // unsigned longs
                        byte[] bytes = new byte[] { (byte) ((v >> 24) & 0xff),
                                (byte) ((v >> 16) & 0xff),
            ...
            if (value >= MP_NEGATIVE_FIXNUM_INT
                    && value <= MP_NEGATIVE_FIXNUM_INT + MAX_5BIT) {
                return (byte) value;
            } else if (value >= MP_FIXARRAY_INT
                    && value <= MP_FIXARRAY_INT + MAX_4BIT) {
                return unpackList(value - MP_FIXARRAY_INT, in, options);
            } else if (value >= MP_FIXMAP_INT
                    && value <= MP_FIXMAP_INT + MAX_4BIT) {
                return unpackMap(value - MP_FIXMAP_INT, in, options);
            } else if (value >= MP_FIXRAW_INT
                    && value <= MP_FIXRAW_INT + MAX_5BIT) {
                return unpackRaw(value - MP_FIXRAW_INT, in, options);
            } else if (value <= MAX_7BIT) {// MP_FIXNUM - the value is value as
                                           // an int
                return value;
            } else {
                throw new InvalidMsgPackDataException(
                        "Input contains invalid type value");
            }
    ...
    protected static List unpackList(int size, DataInputStream in, int options)
            throws IOException {
        if (size < 0)
            throw new InvalidMsgPackDataException(
                    "Array to unpack too large for Java (more than 2^31 elements)!");
    ...
    protected static Map unpackMap(int size, DataInputStream in, int options)
            throws IOException {
        if (size < 0)
            throw new InvalidMsgPackDataException(
                    "Map to unpack too large for Java (more than 2^31 elements)!");
    ...
    protected static Object
            unpackRaw(int size, DataInputStream in, int options)
                    throws IOException {
        if (size < 0)
            throw new InvalidMsgPackDataException(
                    "byte[] to unpack too large for Java (more than 2^31 elements)!");
